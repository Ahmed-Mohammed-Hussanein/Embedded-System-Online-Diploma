...
static void send(LCD_controlConfig_t *config, uint8_t _Byte, uint8_t rsState)
{
	//while(readBusyFlag(config) == 1);

	MCAL_GPIO_writePin(config->LCD_controlPort, config->LCD_RS_pin, rsState);
	MCAL_GPIO_writePin(config->LCD_controlPort, config->LCD_RW_pin, GPIO_STATE_LOW);
...
void HAL_LCD_Init(LCD_controlConfig_t *config)
{
	...
	pin.GPIO_PinNumber		= config->LCD_EN_pin;
	MCAL_GPIO_Init(config->LCD_controlPort, &pin);

	pin.GPIO_PinNumber		= config->LCD_RS_pin;
	MCAL_GPIO_Init(config->LCD_controlPort, &pin);

	pin.GPIO_PinNumber		= config->LCD_RW_pin;
	MCAL_GPIO_Init(config->LCD_controlPort, &pin);

	pin.GPIO_PinNumber		= LCD_DB4_pin;
	MCAL_GPIO_Init(LCD_dataPort, &pin);

	pin.GPIO_PinNumber		= LCD_DB5_pin;
	MCAL_GPIO_Init(LCD_dataPort, &pin);

	pin.GPIO_PinNumber		= LCD_DB6_pin;
	MCAL_GPIO_Init(LCD_dataPort, &pin);

	pin.GPIO_PinNumber		= LCD_DB7_pin;
	MCAL_GPIO_Init(LCD_dataPort, &pin);

#if (DATA_LINE == _8_BIT_DATA_LINE)

	pin.GPIO_PinNumber		= LCD_DB0_pin;
	MCAL_GPIO_Init(LCD_dataPort, &pin);

	pin.GPIO_PinNumber		= LCD_DB1_pin;
	MCAL_GPIO_Init(LCD_dataPort, &pin);

	pin.GPIO_PinNumber		= LCD_DB2_pin;
	MCAL_GPIO_Init(LCD_dataPort, &pin);

	pin.GPIO_PinNumber		= LCD_DB3_pin;
	MCAL_GPIO_Init(LCD_dataPort, &pin);
...
{
	WRITE_REG_POS(EntryModeSet, 0x3, S_BIT, SHIFT_LEFT);
	send(config, EntryModeSet, COMMAND);
}

void HAL_LCD_noAutoShiftLeft(LCD_controlConfig_t *config)
{
	WRITE_REG_POS(EntryModeSet, 0x3, S_BIT, NO_SHIFT_INC);
	send(config, EntryModeSet, COMMAND);
}

void HAL_LCD_autoShiftRight(LCD_controlConfig_t *config)
{
	WRITE_REG_POS(EntryModeSet, 0x3, S_BIT, SHIFT_RIGHT);
	send(config, EntryModeSet, COMMAND);
}
...
{
	WRITE_REG_POS(CursorDisplayShift, 0x3, R_L_BIT, MOVE_CURSOR_LEFT);
	send(config, CursorDisplayShift, COMMAND);
}

void HAL_LCD_moveCursorRight(LCD_controlConfig_t *config)
{
	WRITE_REG_POS(CursorDisplayShift, 0x3, R_L_BIT, MOVE_CURSOR_RIGHT);
	send(config, CursorDisplayShift, COMMAND);
}

void HAL_LCD_moveDisplayLeft(LCD_controlConfig_t *config)
{
	WRITE_REG_POS(CursorDisplayShift, 0x3, R_L_BIT, MOVE_DISPLAY_LEFT);
	send(config, CursorDisplayShift, COMMAND);	
}

void HAL_LCD_moveDisplayRight(LCD_controlConfig_t *config)
{
	WRITE_REG_POS(CursorDisplayShift, 0x3, R_L_BIT, MOVE_DISPLAY_RIGHT);
	send(config, CursorDisplayShift, COMMAND);
}
