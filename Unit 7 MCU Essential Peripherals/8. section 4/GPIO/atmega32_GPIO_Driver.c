...
void MCAL_GPIO_Init(GPIO_TypeDef *GPIOx, GPIO_PinConfig_t *PinConfig)
{
	switch(PinConfig->GPIO_PinMode)
	{
		...
		CLEAR_REG(GPIOx->DDRx, PinConfig->GPIO_PinNumber);
		break;
		
		case GPIO_MODE_OUTPUT:
		SET_REG(GPIOx->DDRx, PinConfig->GPIO_PinNumber);
		break;
		
		case GPIO_MODE_INPUT_PUR:
		CLEAR_REG(GPIOx->DDRx, PinConfig->GPIO_PinNumber);
		SET_REG(GPIOx->PORTx, PinConfig->GPIO_PinNumber);
		break;
	}
}
...
void MCAL_GPIO_writePin(GPIO_TypeDef *GPIOx, uint8_t PinNumber, uint8_t value)
{
	...
	{
		case GPIO_STATE_LOW:
		CLEAR_REG(GPIOx->PORTx, PinNumber);
		break;
		
		case GPIO_STATE_HIGH:
		SET_REG(GPIOx->PORTx, PinNumber);
		break;
	}
}
...
{
	switch(READ_REG(GPIOx->PINx, PinNumber))
	{
		case GPIO_STATE_LOW:
